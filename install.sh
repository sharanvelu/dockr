...
DOCKR_DIR_MINIO="${HOME}/Documents/minio/data"
...
finalize_setup() {
    ...
    # Setup .dockr data directory
    if [ ! -d "${DOCKR_DIR_DATA}" ]; then
        mkdir "${DOCKR_DIR_DATA}"
    fi

    # Setup Minio data directory
    if [ ! -d "${DOCKR_DIR_MINIO}" ]; then
        mkdir "${DOCKR_DIR_MINIO}"
    fi
    ...
    if [ ! -f "${DOCKR_FILE_CONFIG}" ]; then
        ...
        echo "minio=0" >> "${DOCKR_FILE_CONFIG}"
        echo "opensearch=0" >> "${DOCKR_FILE_CONFIG}"

        echo -e "\n"
        echo -e "------------ NOTE ------------"
        echo -e "${PROCESS}The asset Containers ${GREEN}MySQL, postgreSQL, Redis, Minio and OpenSearch${CLR} are ${RED}disabled${CLR} by default."
...
add_permissions() {
    ...
    # Give permission to Minio Data directory to the current user
    sudo chown -R $(whoami) "${DOCKR_DIR_MINIO}"
}
